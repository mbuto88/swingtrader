...
def scaleDataBuildModel(symbol, filename):
    ...
    # Fit the scalers
    scaled_input = input_scaler.fit_transform(df[["Date", "Open", "High", "Low", "Close", "Adj Close", "Volume"]])
    scaled_output = output_scaler.fit_transform(df[["Date", "Open", "High", "Low", "Close"]])

    # Reshape data for LSTM [samples, timesteps, features]
    X, y, times = [], [], []
    for i in range(60, len(scaled_input)):
        X.append(scaled_input[i - 60:i])
        y.append(scaled_output[i])  # Predicting 'open', 'high', 'low', 'close', 'adj close'
    ...
    for train_index, test_index in tscv.split(X):
        ...
        next_day_time = df['Date'].iloc[-1] + 86400
        next_day_time_in_human_format = unix_timestamp_to_mmddyyyy(next_day_time)
        # Display the next day's prediction with its corresponding time
        print(f"Date: {next_day_time_in_human_format}, Predicted Open: {next_day_prediction_transformed[0][0]}, High: {next_day_prediction_transformed[0][1]}, Low: {next_day_prediction_transformed[0][2]}, Close: {next_day_prediction_transformed[0][3]}, Adj Close: {next_day_prediction_transformed[0][4]}")
        day_delta = next_day_prediction_transformed[0][3] - next_day_prediction_transformed[0][0]

    return StockPrediction(symbol, next_day_time_in_human_format, day_delta, next_day_prediction_transformed[0][1],
                           next_day_prediction_transformed[0][0], next_day_prediction_transformed[0][3],
                           next_day_prediction_transformed[0][4])
...
def scaleDataBuildModelV2(symbol, filename):
    try:
        ...
        scaled_output = output_scaler.fit_transform(df[["Open", "High", "Low", "Close"]])
        ...
        for train_index, test_index in tscv.split(X):
            ...
            next_day_time2 = pd.to_datetime(df['Date'].iloc[-1]) + pd.DateOffset(days=1)
